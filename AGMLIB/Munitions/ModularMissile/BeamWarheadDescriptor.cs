...
public class BeamWarheadDescriptor : BaseWarheadDescriptor
{

    protected StatValue _burstDuration;
    //ContinuousRaycastMuzzle
    //TurretedContinuousWeaponComponent

    [SerializeField]
    private readonly Vector3 _fusedimensions = Vector3.one;
    [SerializeField]
    ...
    private List<int> Weights => _scalingvalues.ConvertAll<int>(scalingvalue => (int)Math.Round(scalingvalue.GetValue(base._weightedSocketSize)));
    ...
    {
        return TriggerDetonate(runtime, hitObject, hitInfo);
    }
